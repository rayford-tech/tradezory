...
export function TradingCalendar({ dayMap, rawTrades }: TradingCalendarProps) {
  ...
  const selectedTrades = selectedDay
    ? rawTrades.filter((t) => t.closeTime && format(new Date(t.closeTime), "yyyy-MM-dd") === selectedDay)
    : [];
    ...
    <div className="space-y-5">
      <div className="flex items-center justify-between">
        ...
        <div className="flex items-center gap-2">
          ...
          <button onClick={() => setCurrent((d) => subMonths(d, 1))} className="p-1.5 rounded-lg border border-zinc-700 hover:bg-zinc-800 text-zinc-400">
            <ChevronLeft className="h-4 w-4" />
          </button>
          <button onClick={() => setCurrent(new Date())} className="px-3 py-1.5 rounded-lg border border-zinc-700 hover:bg-zinc-800 text-xs text-zinc-400">
            Today
          </button>
          <button onClick={() => setCurrent((d) => addMonths(d, 1))} className="p-1.5 rounded-lg border border-zinc-700 hover:bg-zinc-800 text-zinc-400">
              ...
              return (
                <button
                  ...
                  className={cn(
                    ...
                    isSelected ? "bg-indigo-600/10 border-indigo-500/30" : "hover:bg-zinc-800/40",
                ...
                >
                  ...
                  {data && (
                    <>
                      ...
                      <div className="text-[10px] text-zinc-600">{data.trades}t</div>
                    </>
                  )}
                </button>
              );
            ...
            <>
              ...
              <p className="text-xs text-zinc-500 mb-4">
                ...
                <span className={dayMap[selectedDay]?.pnl >= 0 ? "text-emerald-400" : "text-red-400"}>
                  {formatCurrency(dayMap[selectedDay]?.pnl ?? 0)}
                </span>
              </p>
              <div className="space-y-2">
                {selectedTrades.map((t) => {
                  ...
                  return (
                    <a key={t.id} href={`/journal/${t.id}`} className="flex items-center justify-between rounded-lg border border-zinc-700 px-3 py-2.5 hover:border-indigo-500/30 transition-colors">
                      <div>
